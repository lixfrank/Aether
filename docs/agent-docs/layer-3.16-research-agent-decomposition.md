...
research.md (~178行)
  ...
  └─ Path 3: invoke /research-coordinator skill tool
        ...
        │   §4a Audit-Repair通用机制 (含Repair Pre-backup via scripts/backup_repair.sh)
        ...
        └─ references/
            ...
            │ phase-detail-tables.md (477行) — 参数化表格+完整dispatch prompts
            │   (含完整skip/rollback/consistency规则
...
| `.aether/skills/research-coordinator/references/phase-detail-tables.md` | 0(新建)  | 477      | ~425±30    | +477     |
...
| 4a  | Audit-Repair通用机制            | ~80        | ~45      | 598-664(合并精简)                                  | 通用循环机制1处+Repair Pre-backup via scripts/backup_repair.sh. 条件分支路由不展开——引用references/phase-routing.md §Audit-1 Routing / §Audit-2 Routing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 5   | Debate Loop流程骨架             | ~80        | ~25      | 936-1162(高度精简)                                 | 5 sub_phase顺序+Repair Pre-backup via scripts/backup_repair.sh+Sub-phase Output Verification(check_file_updated MCP)+引用references/phase-routing.md §Debate Sub-phase Routing+§Debate Round Termination+§Debate Error Handling+§Repair Digest Processing+§User Rejection Options。dispatch prompt模板移到references/phase-detail-tables.md                                                                                                                                                                                                                                                                                                      |
...
| Domain Mode Determination     | ~15        | ~15      | 1254-1264                                                                                                  | domain_mode由framing digest直接输出+fallback规则                                                                                                                                                 |
| Git Rollback Protocol(统一)   | ~25        | ~25      | 480-486+1242-1248+1452+443-457                                                                             | 统一rollback操作步骤(含state.json排除协议)                                                                                                                                                       |
...
**总计: 477行** (原预估~425±30行。实现优化: 删除Audit-Repair参数差异表(Q1冗余)、Repair Pre-backup File Lists+Debate Repair Pre-backup(Q2改用scripts/backup_repair.sh)、State Consistency Check(Q5与SKILL.md重复)、Persistence Directory Conventions(Q6属subagent知识), 共-89行)
...
| Repair Crash Recovery (audit+debate) | ~10        | ~10      | 926-934       | repair worker crash→restore from backup_repair.sh backups→retry (统一audit+debate) |
...
**合并后**: SKILL.md中通用机制(~40行) + scripts/backup_repair.sh(~68行, 含文件列表) = ~108行,替代250行,减少57%. 参数差异表已删除(Q1冗余——repair files 硬编码在 backup_repair.sh, repair skill/next phase/plan_number 由 state.json + routing rules 确定).
...
3. issues_found>0 + repair_count<3 → dispatch repair worker (run scripts/backup_repair.sh first)
...
| 443-457                   | State Consistency Check                          | → coordinator SKILL.md §2(digest processing后续) — inlined, no separate reference section                                                             |
...
| 1596-1616                 | Persistence directory conventions                | → subagent skills (audit/repair/paper-search/literature-landscape-scan 自身定义各自的 I/O 路径; coordinator 不承载 subagent 路径知识)                 |
...
| git rollback模式  | 第480-486行+第1242-1248行+第1452行+第443-457行(State Consistency) | 合并到references/phase-detail-tables.md §Git Rollback Protocol+references/phase-routing.md §Checkpoint Rollback; State Consistency Check内联到SKILL.md §2 |
...
| 5   | §4a Audit-Repair通用机制           | 通用循环机制1处+Repair Pre-backup via scripts/backup_repair.sh. 条件分支引用references/phase-routing.md                                                                                                                                               |
| 6   | §4b Audit-3特殊路由概述            | 4条分支概述+完整条件分支+dispatch prompts引用references/phase-routing.md+references/phase-detail-tables.md                                                                                                                                            |
| 7   | §5 Debate Loop流程骨架             | 5 sub_phase顺序+Repair Pre-backup via scripts/backup_repair.sh+Sub-phase Output Verification+引用references/phase-routing.md                                                                                                                          |
...
| 20  | phase-detail-tables.md | All Dispatch Prompt Templates完整(含3种audit repair+framing retry+landscape supplement+frontier_problem PoC question+debate 5 sub_phase+execution+execution re-dispatch+analysis_checkpoint rollback re-dispatch+landscape audit_1 injection prompt) |
| 21  | phase-detail-tables.md | Domain Mode规则完整(framing digest domain_mode字段直接输出+fallback)                                                                                                                                                                                 |
| 22  | phase-detail-tables.md | Git Rollback Protocol统一替代3处重复(含state.json排除协议)                                                                                                                                                                                           |
| 23  | phase-detail-tables.md | Phase Display Name Mapping完整(state.json phase→display name+dynamic parameter)                                                                                                                                                                      |
