...
- 新增 audit 质量门：产出后 worker 跑 scripts + dispatch research-audit sub-subagent 做语义审计（design doc §7.4, newlayer-7 §1）
...
| Step1 中 convention_lock_status 检查                 | convention 检查移到 framing Step9（analysis 阶段无需物理约定锁定，analysis 是通用分析） |
...
- 回退重入: 更新 Current Understanding + Phase History
```
