...
- PASS → Qn status 改 resolved, 写 conclusion 到 research_state.md
...
- 每个 Wave 完成后，dispatch research-audit sub-subagent 检查跨问题一致性
...
- MUST: 每 Wave 完成后 dispatch research-audit sub-subagent 检查跨问题一致性
...
- FORBIDDEN: 修改 PLAN.md claims（method 字段修改由 primary agent 在 dispatch 前处理）
```
...
3. Update research_state.md: 各 Qn status 更新, Phase History 追加 execution ✓, Failed Attempts 追加失败方法
4. dispatch research-audit sub-subagent 做最终质量门（审汇总质量、跨问题一致性）
