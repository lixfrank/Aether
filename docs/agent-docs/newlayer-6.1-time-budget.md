...
**核心洞察**：时间比轮数更贴合研究本质——研究的"工作量"是时间而非轮次。check_time_budget.py 追踪已用时间并返回事实，agent 读时间事实 + verdict + 研究上下文自行判断何时该停。脚本不做决策，避免用决策表替代 agent 判断（违背 §6.1"原则非施工步骤"设计哲学）。

**具体决策理由**：

- **per-question 而非 phase 级**：每个问题难度不同，独立预算更合理。phase 级总预算会让简单问题挤占复杂问题的时间。
- **移除所有固定轮数上限（cycle max 3 + 文件验证 max 2），纯时间驱动**：时间预算是"该 question 上还能试多久"的单一控制，适用于所有失败点（文件验证不合格 / verdict FAIL/PARTIAL）。保留独立 max 会引入两套终止机制，逻辑割裂。agent 在 min 时间前不因失败停止、接近 max 且无进展则暂停、之间自行判断——原则足够防止无意义循环，不需要 zone→action 决策表。
...
- **不新增 status**：超时暂停的 question 不标 resolved，问题分析写入 Last Phase Result。paused 即 needs_attention 回传，check_verification.py 不受影响（只查 resolved claims）。question 的具体 status 由 agent 据情况选择准确描述的标签（如 open/partial/failed/...），唯一确定性约束是 `resolved` 须有 ver 路径。
- **脚本只提供事实，agent 做决策**：check_time_budget.py 返回 elapsed_minutes / min_minutes / max_minutes，不做 zone 判定、不返回 action。agent 读时间事实 + verdict + 研究上下文自行判断是否 retry/pause。这避免用决策表替代 agent 判断——§6.1 明确要求"注明 agent 必须做什么…agent 据情况灵活推进"，zone→action 映射表违背此原则。
...
| 段落                                                      | 理由                                                                                  |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| Terminology 中 `cycle` 定义                               | 被 `time budget` 取代                                                                 |
| 据 verdict 决策中 `cycle < 3 → 重试 / cycle = 3 → failed` | 被原则驱动决策取代（agent 读 verdict + 时间事实 + 上下文自行判断）                    |
| 文件验证中 `重跑对应 subagent（max 2）`                   | 文件验证失败也纳入时间预算，无独立 max                                                |
| `cycle = 3 → Qn status 改 failed` 自动路径                | 超时改为 needs_attention 暂停（非自动 failed）；question 是否 failed 由人类审查后决定 |
| zone → action 映射表（below_min/between/above_max）       | 决策表替代 agent 判断——违背"原则非施工步骤"设计哲学；agent 读时间事实自行决策         |
| "自判"独立节（between 区两条软性指引）                    | agent 判断是全场景的，不需要被限定在某个 zone 内                                      |
| T3 中人类响应枚举选项                                     | 违背 §5.1"不弹 question 选项列表"——暂停后人类自由对话，响应由 primary agent 处理      |
...
**check**：读 .timing.json 的 started_at + min/max（无则用默认 60/1440）→ 计算 elapsed → 返回时间事实。无记录返回 null 字段。**脚本不做决策**——agent 读时间事实后自行判断是否继续。
...
（started_at / elapsed_minutes 为 null 表示未 reset）

内部用秒做 elapsed 计算，输出转 minutes（1 位小数）。
...
### T2. autoresearch SKILL.md 时间预算 + 决策

**时间预算节**：

- 处理 Qn 前（首次或 re-examine）调 `reset`（归零计时，可选 --min/--max）
- 每次 subagent 产出验证后（文件验证 或 verdict 验证）调 `check` 获取已用时间
...
- 不是 PASS → 读 verdict 的失败原因和 evidence，判断：
  - 有未试的方向或可调整的方法 → 重跑对应 subagent（调整 prompt 指出问题，不得机械重复）
  - 根本性问题（方法不适用 / claim 与已知定律矛盾）→ 暂停（见 §暂停）
  - 依赖结论不可用 → 判断是否影响当前 question，决定继续/跳过/暂停
- 当你停止处理某个 question 且未将其标为 resolved 时，将已试方法和失败原因记入 Failed Attempts

### T3. 暂停行为

当你无法在某个 question 上继续推进时：

1. 已试方法和失败原因记入 research_state.md 的 Failed Attempts（method + 失败原因 + 排除方向）
...
- MUST: 处理 Qn 前（首次或 re-examine）调 reset；每次 subagent 产出验证后（文件验证或 verdict）调 check 获取已用时间
- MUST: 停止处理某个 question 且未标 resolved 时，记录已试方法到 Failed Attempts + 写问题分析到 Last Phase Result + 立即回传 needs_attention
...
- **check_verification.py**：无需改——超时暂停的 question 不标 resolved，check_verification 只查 resolved claims
...
- autoresearch SKILL.md：~132行 → ~160行（+时间预算节 + 据 verdict 决策节 + 暂停节 + 硬约束更新）
- 新增 check_time_budget.py：~170行（确定性脚本，PEP 723 header，reset/check 两动作，minutes 单位，只返回时间事实不做决策）
...
- [ ] check_time_budget.py 只返回时间事实（elapsed_minutes / min_minutes / max_minutes），**不做决策**（无 zone / action / remaining 字段）
- [ ] 时间预算是 agent 判断的输入，非决策表：agent 读时间事实 + verdict + 上下文自行决定 retry/pause
- [ ] 在 min 时间之前不因失败停止；接近 max 且无进展则暂停；之间自行判断
- [ ] 暂停：记录已试方法到 Failed Attempts + 写问题分析到 Last Phase Result（status=needs_attention）+ 立即回传 needs_attention，不处理其他 question
- [ ] SKILL.md 不含 zone→action 映射表 / "自判"独立节 / T3 人类响应枚举选项
...
- [ ] 超时暂停的 question 不标 resolved，分析写入 Last Phase Result

### 脚本强制验收

- [ ] `不得存在` autoresearch SKILL.md 中的 `cycle` / `max 3` / `max 2` 终止逻辑引用
- [ ] `不得存在` autoresearch SKILL.md 中的 `zone` / `below_min` / `above_max` / `must_continue` / `self_judge` / `hard_stop` 引用
...
- [ ] `check_time_budget.py reset` 归零 started_at + 存 --min/--max 到 .timing.json；校验 min≤max（用默认值填充缺失参数后校验）
- [ ] `check_time_budget.py check` 输出含 `elapsed_minutes` + `min_minutes` + `max_minutes`；**不含** `zone` / `action` / `remaining_*`
